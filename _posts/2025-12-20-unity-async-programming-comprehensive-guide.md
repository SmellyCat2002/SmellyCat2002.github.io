...
本文重构核心逻辑：从「新手避坑口诀」切入，到「进阶本质理解」，再到「场景落地应用」「原理深挖」「面试备战」，形成完整学习闭环，帮你彻底分清协程、async/await、线程的用法与差异。
...
3. 关键认知：是“async函数自我挂起”，非主线程主动停止；主线程会先完成手头同步任务（如Update），再回来执行await后代码。
