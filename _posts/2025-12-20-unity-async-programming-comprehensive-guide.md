...
- **任务/场景类型**：Unity资源/场景加载（Resources.LoadAsync）、UnityWebRequest
  **推荐方案**：协程 / UniTask
  **核心说明**：使用Unity原生异步API，无需手动开线程；等待型任务，底层已处理多线程调度

- **任务/场景类型**：游戏对象操作（移动、销毁、UI更新）
  **推荐方案**：主线程同步 / 协程
  **核心说明**：直接操作Unity对象必须在主线程执行，非耗时任务无需异步
...
- **任务/场景类型**：文件I/O（本地文件读写）
  **推荐方案**：UniTask
  **核心说明**：通用等待型任务，.NET API原生支持异步，无需手动开线程

---

### 3. 计算密集型场景

- **任务/场景类型**：纯计算任务（解析大JSON/XML、A*寻路、噪声图生成）
  **推荐方案**：UniTask + Task.Run
  **核心说明**：持续消耗CPU的通用操作，需用Task.Run开子线程计算，UniTask等待结果回调主线程
