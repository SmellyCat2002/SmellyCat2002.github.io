...
**内容**：Boss 突进至玩家左侧，连续挥出两段正面拳击，第三段转为以自身为圆心的圆形 AOE。第三段 AOE 释放后进行距离判定（以距离 Boss 最近的玩家为准）：若最近玩家仍超出一定安全距离（比 AOE 范围更远），Boss 会追加一道远程冲击波攻击；若最近玩家在 AOE 范围内或刚脱离 AOE 范围但未达到安全距离，则无追加。追加冲击波后 Boss 才进入硬直。硬直恢复后 Boss 会再接一个圆形 AOE 作为收招，形成"连打-距离判定-冲击波-硬直-收招"的完整节奏。

**应对方法**：前两段正面攻击通过侧向翻滚躲避；第三段圆形 AOE 时需保持在 AOE 范围内或刚脱离即可，不要跑得太远，这样不会触发冲击波追加；硬直窗口是最佳输出时机，全队集中火力打一轮爆发；硬直恢复后的收招 AOE 仍需及时脱离。

**乐趣**：三拳节奏感强，撑过去后 Boss 进入硬直，全队获得充足的输出窗口；距离判定机制要求玩家控制在 AOE 范围内，在"躲避伤害"和"避免触发追加"之间把握分寸。
