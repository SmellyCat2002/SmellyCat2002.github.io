...
**内容**：Boss 突进至玩家左侧，连续挥出两段正面拳击，第三段转为以自身为圆心的圆形 AOE。第三段 AOE 释放后立即进行距离判定：若玩家距离过远（超出圆形 AOE 范围），Boss 会追加一道远程冲击波攻击；若玩家在 AOE 范围内则无追加。追加冲击波后 Boss 才进入硬直。硬直恢复后 Boss 会再接一个圆形 AOE 作为收招，形成"连打-距离判定-冲击波-硬直-收招"的完整节奏。

**应对方法**：前两段正面攻击通过侧向翻滚躲避；第三段圆形 AOE 时需保持在 AOE 范围内（不要跑太远），这样不会触发冲击波追加；硬直窗口是最佳输出时机，全队集中火力打一轮爆发；硬直恢复后的收招 AOE 仍需及时脱离。
